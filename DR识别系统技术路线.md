...
- **可选 ME 增强:** MAPLES-DR —— 若 ME 头精度成为瓶颈,这是注入更多 ME 监督的唯一途径。
...
7. **ME 头的局限**:无外部 ME 标签,ME 头几乎无法从外部数据获益。若 M2 精度成为瓶颈,唯一途径是引入 **MAPLES-DR** 补充 ME/病灶监督。

> 推荐执行顺序:QWK 评测 → 类别均衡/LDAM 损失 → 若 R3/M2 仍弱 → 解耦重训分类头 + 外部数据 / MAPLES-DR。
...
- **消融实验**(报告核心):
  ...
  5. 普通 softmax vs 序数头;
  6. 有/无长尾策略。
- **可解释性(加分项)**:Grad-CAM / 注意力热力图,展示模型关注的病灶区域是否与临床一致。
...
6. **(可选)进阶**:引入 MAPLES-DR 增强 ME;或加生成式/VLM 分支做异常热力图与文本化报告作为"高级特性"。
...
| ME 头数据稀少且长尾严重 | M2 召回低 | 更强增广+均衡损失;必要时引入 MAPLES-DR |
...
- **MAPLES-DR**:在 MESSIDOR 子集上提供像素级病灶标注与 DR/ME 分级的外部数据集,是补充 ME 监督的唯一来源。
